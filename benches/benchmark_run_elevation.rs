...
fn benchmark_run_elevation(_c: &mut Criterion) {
    let mut config = base_config();
    config.output_pbf = Some(PathBuf::from("target/tmp/output-integration-bench-run_elevation.pbf"));
    ...
    criterion.bench_function("run_elevation", |b| {
        b.iter(|| {
...
criterion_group!(benches, benchmark_run_elevation);
