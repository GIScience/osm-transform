...
fn benchmark_run_node_filtering(_c: &mut Criterion) {
    let mut config = base_config();
    config.output_pbf = Some(PathBuf::from("target/tmp/output-integration-bench-run_node_filtering.pbf"));
    ...
    criterion.bench_function("run_node_filtering", |b| {
        b.iter(|| {
...
criterion_group!(benches, benchmark_run_node_filtering);
