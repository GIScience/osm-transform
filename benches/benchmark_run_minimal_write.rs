...
fn benchmark_run_minimal_write(_c: &mut Criterion) {
    let mut config = base_config();
    config.output_pbf = Some(PathBuf::from("target/tmp/output-integration-bench-run_minimal_write.pbf"));
    let mut criterion = Criterion::default().sample_size(100); // Set sample size for this benchmark

    criterion.bench_function("run_minimal_write", |b| {
        b.iter(|| {
...
criterion_group!(benches, benchmark_run_minimal_write);
