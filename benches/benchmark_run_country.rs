...
fn benchmark_run_country(_c: &mut Criterion) {
    let mut config = base_config();
    config.output_pbf = Some(PathBuf::from("target/tmp/output-integration-bench-run_country.pbf"));
    ...
    criterion.bench_function("run_country", |b| {
        b.iter(|| {
...
criterion_group!(benches,benchmark_run_country);
